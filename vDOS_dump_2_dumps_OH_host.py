...
natom_real = nH_host + nO_host
natom_host = nH_host + nO_host + nSi_host


fout1 = open( "OH_dump_pos.lmptraj", "w")
fout2 = open( "OH_dump_vel.lmptraj", "w")
fout3 = open( "MD_host.xyz", "w")
...
while True:
    ...
    tmp = f.readline()
    tmp2 = tmp.split()
    Lx = float(tmp2[1])-float(tmp2[0])
    fout1.write("%s" %( tmp ))
    fout2.write("%s" %( tmp ))
    tmp = f.readline()
    tmp2 = tmp.split()
    Ly = float(tmp2[1])-float(tmp2[0])
    fout1.write("%s" %( tmp ))
    fout2.write("%s" %( tmp ))
    tmp = f.readline()
    tmp2 = tmp.split()
    Lz = float(tmp2[1])-float(tmp2[0])
    fout1.write("%s" %( tmp ))
    fout2.write("%s" %( tmp ))
    ...
    fout3.write("%6d\n" %( natom_host ))
    fout3.write("%15.6f %15.6f %15.6f\n" %( Lx, Ly, Lz ))

    # print Oxygen first, with type 1
    index_begin = nH_host + 2*nwat
    index_end   = nH_host + 2*nwat + nO_host
    for i in range(index_begin, index_end):
        ...
        fout3.write("%4s %15.6f %15.6f %15.6f\n" %( "O", x[i], y[i], z[i] ))
    ...
    for i in range(index_begin, index_end):
        ...
        fout3.write("%4s %15.6f %15.6f %15.6f\n" %( "H", x[i], y[i], z[i] ))
    # Finally, print Silicon
    index_begin = nH_host + nO_host + 3*nwat
    index_end   = nH_host + nO_host + 3*nwat + nSi_host
    for i in range(index_begin, index_end):
        fout3.write("%4s %15.6f %15.6f %15.6f\n" %( "Si", x[i], y[i], z[i] ))
