...
import argparse
parser = argparse.ArgumentParser(description='subtract atomic energies of file xyzfes')
...
def number_CHNO(global_types, molecule_types, molecule_nums):
    res = np.zeros(len(global_types), dtype=int)
    for id_tmp in range(len(molecule_types)):
        atom_loc = np.where(global_types == molecule_types[id_tmp])
        res[atom_loc] = molecule_nums[id_tmp]
    return res

istruc = 0
global_types = np.array(atom_type)
...
while True:
    ...
    molecule_types, molecule_nums = np.unique(atomList, return_counts=True)
    Amatrix = np.append(Amatrix, number_CHNO(global_types, molecule_types, molecule_nums) )
    istruc += 1
f.close
