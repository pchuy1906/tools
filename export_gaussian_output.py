...
parser.add_argument("--do_export_force", action='store_true', help='export force from output')
...
method              = args.method
do_export_force     = args.do_export_force

print ("do_export_force", do_export_force)
...
while True:
    ...
    #if (len(line)==2) and (line[0] == 'Input') and (line[1] == 'orientation:'):
    if (len(line)==2) and (line[0] == 'Standard') and (line[1] == 'orientation:'):
    ...
    if found_energy and (not still_read_xyz) and (not still_read_fxyz) and do_export_force:
        nconfig += 1
        found_energy    = False
        still_read_xyz  = True
        still_read_fxyz = True
    ...
    if found_energy and (not still_read_xyz) and (not do_export_force):
        nconfig += 1
        found_energy    = False
        still_read_xyz  = True
        still_read_fxyz = True
