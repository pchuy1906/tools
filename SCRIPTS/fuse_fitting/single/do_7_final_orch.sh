...
output_opt_ff = "../output_opt_ff/"

# Required input files for this pair
data_file_1 = f"{output_opt_ff}/opt_data_{mol_name}.lammps"
style_file_1 = f"{output_opt_ff}/style_{mol_name}.json"

topo_mol = potential._parse_to_topology(data_file_1, style_file_1)

monomer_1 = potential.extract_monomer(topo_mol)
monomer_2 = monomer_1.copy()
monomer_2.arrays[MOL_ID_KEY] = np.full(len(monomer_2), 2)


dimers, rcoms = gen_dimers(monomer_1, monomer_2)

path_de_lmp = f"LAMMPSSimulator/dimer_{mol_name}/EINT.txt"
de_ff = np.loadtxt(path_de_lmp)

path_de_vasp = f"VaspOracle/dimer_{mol_name}/EINT.txt"
...
for mole_ratio in mole_ratios:
    for density in densities:
        ...
        path_de_lmp = f"LAMMPSSimulator/condensed_{mol_name}_{mole_ratio}_{density}/EINT.txt"
        de_ff = np.loadtxt(path_de_lmp)
        path_de_vasp = f"VaspOracle/condensed_{mol_name}_{mole_ratio}_{density}/EINT.txt"
        de_dft = np.loadtxt(path_de_vasp)

        dump_file = f"LAMMPSSimulator/condensed_{mol_name}_{mole_ratio}_{density}/00000/initial.dump"
...
topo_mol.ij_pair_coeffs = ij_pair_coeffs
potential._write_potential_to_file(
    topo_mol,
    ".",
    write_pair_coeffs=True,
    write_ij_pair_coeffs=True,
)

# Move optimized files into the optimization folder
os.system(f"mv data.lammps opt_data_{mol_name}.lammps")
os.system(f"cp {style_file_1}  style_{mol_name}.json")
