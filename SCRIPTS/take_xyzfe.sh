...
grep -e "in kB"  ${OUTCAR} | awk '{printf "%15.9f %15.9f %15.9f\n", $3*'"$kbar2GPa"', $4*'"$kbar2GPa"', $5*'"$kbar2GPa"'}' > STRESS_TENSOR.dat
...
rm -rf ${OUTPUT}
rm -rf DFT.FORCE_ENERGY ID_CHECK DFT.STRESS_TENSOR

for iconfig in ${NSTRUC_NEED}; do
  echo ${natoms} >> ${OUTPUT}
  sed -n ${iconfig},${iconfig}p  ENERGIES.dat >  thisENERGY

  sed -n ${iconfig},${iconfig}p  STRESS_TENSOR.dat | fmt -1 >> DFT.STRESS_TENSOR

  paste  boxABC  thisENERGY atomSym.dat >> ${OUTPUT}
  ...
  while read line; do
    echo $line | awk '{printf "%15.9f %15.9f %15.9f\n", $4*'"$eV2kcal"', $5*'"$eV2kcal"', $6*'"$eV2kcal"'}' | fmt -1 | awk '{print $1, "FORCE"}' >> DFT.FORCE_ENERGY
  done < ATOM.XYZ

  cat thisENERGY | awk '{print $1, "ENERGY"}' >> DFT.FORCE_ENERGY
  cat thisENERGY | awk '{print $1, "ENERGY"}' >> DFT.FORCE_ENERGY
  cat thisENERGY | awk '{print $1, "ENERGY"}' >> DFT.FORCE_ENERGY
