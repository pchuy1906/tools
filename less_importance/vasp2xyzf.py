...
while not EXIT_CONDITION:
	...
	if "free  energy   TOTEN" in LINE:
		...
		if "***" in POT_ENER:
			...
			POT_ENER = float(POT_ENER)* eV2kcalmol # Convert from eV (VASP) to kcal/mol (house_md)
	...
	if START_READ == 1:		# Ignore current (title) line and the "----" line that follows
		if TOTAL_FRAMES%SKIP == 0: # Then we should print this frame
			...
			OFSTREAM.write(`TOTAL_ATOMS` + '\n')
                        ...
                        if INCL_ALLSTR:
			        OFSTREAM.write("NON_ORTHO " + BOXLENS[0] + " " + BOXLENS[1] + " " + BOXLENS[2].rstrip(')'))
                        else:
                                OFSTREAM.write(" " + A[0] + " " + B[1] + " " + C[2])

			
			if INCL_STRESS:
				OFSTREAM.write(" " +`TENSOR_XX `+ " " + `TENSOR_YY` + " " + `TENSOR_ZZ`)
			elif INCL_ALLSTR:
				OFSTREAM.write(" " +`TENSOR_XX `+ " " + `TENSOR_YY` + " " + `TENSOR_ZZ` + " " + `TENSOR_XY` + " " + `TENSOR_ZX` + " " + `TENSOR_YZ`) 
			if INCL_ENERGY:
