...
natom_real = nH_host + nO_host + nSi_host

fout1 = open( "HOST_dump_pos.lmptraj", "w")
fout2 = open( "HOST_dump_vel.lmptraj", "w")
...
while True:
    ...
    # print Si later, with type 3
    index_begin = nH_host + 2*nwat + nO_host + nwat
    index_end   = nH_host + 2*nwat + nO_host + nwat + nSi_host
    for i in range(index_begin, index_end):
        ncount += 1
        fout1.write("%6d %6d %15.6f %15.6f %15.6f\n" %( ncount, 3, x[i], y[i], z[i] ))
        fout2.write("%6d %6d %15.6f %15.6f %15.6f\n" %( ncount, 3, vx[i], vy[i], vz[i] ))
